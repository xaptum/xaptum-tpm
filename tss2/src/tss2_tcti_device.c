...
TSS2_RC
Tss2_Tcti_Device_Init(TSS2_TCTI_CONTEXT *tcti_context,
                      size_t *size,
                      const char *dev_file_path)
{
    if (NULL == tcti_context) {
        *size = sizeof(TSS2_TCTI_CONTEXT_OPAQUE_DEVICE);
    ...
    if (NULL == dev_file_path) {
        strcpy(cast_context->dev_file_path, DEFAULT_DEV_FILE_PATH);
    } else {
        strncpy(cast_context->dev_file_path, dev_file_path, MAX_DEV_FILE_PATH_LENGTH);
        cast_context->dev_file_path[MAX_DEV_FILE_PATH_LENGTH - 1] = 0;  // just in case provided string too long
    }
