...
set_cmdauths(TSS2_SYS_CONTEXT_OPAQUE *sys_context,
             const TSS2_SYS_CMD_AUTHS *cmd_auths_array)
{
    sys_context->cmd_auths_count = cmd_auths_array->cmdAuthsCount;

    if (0 == cmd_auths_array->cmdAuthsCount)
    ...
    for (unsigned i=0; i < cmd_auths_array->cmdAuthsCount; i++) {
        if (!cmd_auths_array->cmdAuths[i])
            return TSS2_SYS_RC_BAD_VALUE;

        marshal_tpms_authcommand(cmd_auths_array->cmdAuths[i], &sys_context->ptr);
    }
...
get_rspauths(TSS2_SYS_CONTEXT_OPAQUE *sys_context,
             TSS2_SYS_RSP_AUTHS *rsp_auths_array)
{
    if (rsp_auths_array->rspAuthsCount != sys_context->cmd_auths_count)
    ...
    for (unsigned i=0; i < rsp_auths_array->rspAuthsCount; i++) {
        if (!rsp_auths_array->rspAuths[i])
            return TSS2_SYS_RC_BAD_VALUE;

        if (0 != unmarshal_tpms_authresponse(&rsp_auths_ptr, &sys_context->remaining_response, rsp_auths_array->rspAuths[i]))
            return TSS2_SYS_RC_MALFORMED_RESPONSE;
    }
